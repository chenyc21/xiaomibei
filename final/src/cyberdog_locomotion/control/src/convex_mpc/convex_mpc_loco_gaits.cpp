...
void ConvexMpcLocoGaits::SetCmd( ControlFsmData< float >& data ) {
    ...
    if ( !enable_terrain_comp_ )
        ...
        pos_cmd_[ 2 ] = user_params_->des_roll_pitch_height[ 2 ] * cos( slope_ ) +  // user params and delta based on x vel
                        WrapRange( fabs( vel_des_robot_( 0 ) * body_height_delta_max_ ) / body_height_delta_vel_max_, 0.0f, body_height_delta_max_ );
    // [PATCH] HEIGHT_BAR override: only treat ctrl_cmd_->pos_des[2] as an
    // absolute body-height target when it falls in the SQUAT range (0.05, 0.20).
    // 上限收到 0.20：locomotion_main.py 的默认 pos_des[2]=0.24 不应触发下蹲；
    // 第四赛段限高杆下蹲一般用 0.10~0.18，仍然落在区间内。
    float h_override = ctrl_cmd_->pos_des[ 2 ];
    bool use_h_override = ( h_override > 0.05f && h_override < 0.20f );
    static int s_hovr_dbg = 0;
    if ( (++s_hovr_dbg) % 500 == 0 ) {
        printf( "[HOVR] ctrl_pos_des[2]=%.3f use=%d pos_des=%.3f min=%.3f max=%.3f\n",
                h_override, (int)use_h_override, pos_des_( 2 ), pos_cmd_min_( 2 ), pos_cmd_max_( 2 ) );
    }
    if ( use_h_override ) {
        pos_cmd_[ 2 ] = h_override;
        float fast_filter = 0.05f;  // ~5x faster than default 0.01
        pos_des_( 2 ) = pos_des_last_( 2 ) * ( 1.0f - fast_filter ) + pos_cmd_[ 2 ] * fast_filter;
        // widen clamp to allow squatting to 0.05m
        pos_des_( 2 ) = WrapRange( pos_des_( 2 ), 0.05f, pos_cmd_max_( 2 ) );
    } else {
