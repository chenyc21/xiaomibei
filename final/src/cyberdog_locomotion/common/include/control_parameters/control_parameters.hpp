...
public:
    ...
    void InitializeMatXd( const std::vector< std::vector< double > >& mat ) {
        ...
        if ( !Check2DArraySize( mat, rows_, cols_ ) ) {
            throw std::runtime_error( StringFormat( "MatXd initialize row&col mismatch on param '%s', expect: (%d, %d), ", name_.c_str(), rows_, cols_ ) );
        }
        set_ = true;
    ...
    void InitializeVecXd( const std::vector< double >& v ) {
        ...
        if ( ( int )v.size() != rows_ ) {
            throw std::runtime_error( StringFormat( "VecXd initialize row mismatch on param '%s', expect %d, "
                                                    "got %d",
                                                    name_.c_str(), rows_, ( int )v.size() ) );
        }
        set_ = true;
        for ( int i = 0; i < rows_; i++ ) {
            value_.vecXd[ i ] = v[ i ];
