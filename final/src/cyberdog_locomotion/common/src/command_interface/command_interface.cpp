...
void CommandInterface::Gamepad2Cmd( long int* control_mode, long int* gait_id, const RobotType& robotType ) {
    ...
    if ( cmd_cur_.mode == MotionMode::kQpStand ) {  // QP stand
        ...
        // body height: leftTriggerAnalog used as absolute height override (m).
        // 0 (or <0.05) means "use default standing height"; >0.05 overrides.
        // Range clamped to [0.05, 0.40] for safety. Used by competition bridge
        // to issue MODE_LOW_WALK (cmd.body_height ∈ [0.05, 0.20] for HEIGHT_BAR).
        {
            float h_ovr = gamepad_cmd_.leftTriggerAnalog;
            float h_def = ( robotType == RobotType::CYBERDOG2 ) ? 0.24f : 0.32f;
            if ( h_ovr > 0.05f ) {
                if ( h_ovr < 0.05f ) h_ovr = 0.05f;
                if ( h_ovr > 0.40f ) h_ovr = 0.40f;
                cmd_cur_.pos_des[ 2 ] = h_ovr;
            } else {
                cmd_cur_.pos_des[ 2 ] = h_def;
            }
        }
        cmd_cur_.step_height[ 0 ] = ( ( robotType == RobotType::CYBERDOG2 ) ? 0.04 : 0.06 );
        // step_height override：rightTriggerAnalog 作为抬脚高度（m），>0.04 时覆盖默认值
        // 用于上独木桥/楼梯等需要高抬腿的场景（默认 0.06m，最高 0.15m）
        {
            float sh_ovr = gamepad_cmd_.rightTriggerAnalog;
            if ( sh_ovr > 0.04f ) {
                if ( sh_ovr > 0.15f ) sh_ovr = 0.15f;
                cmd_cur_.step_height[ 0 ] = sh_ovr;
                cmd_cur_.step_height[ 1 ] = sh_ovr;
            }
        }
    }
