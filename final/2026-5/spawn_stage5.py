#!/usr/bin/env python3
"""
把机器人移动到第五赛段第一段开始位置，便于完整调试 Stage5。
...
def teleport_robot_with_gz_topic():
    ...
    for attempt in range(1, 4):
        ...
        if result.returncode == 0:
            ...
            print("[spawn_stage5] 已通过 gz topic 传送 robot 到第五赛段第一段开始位置。")
            return
...
class Stage5Spawner(Node):
    ...
    def teleport_robot(self):
        ...
        print("[spawn_stage5] 已通过 /set_entity_state 传送 robot 到第五赛段第一段开始位置。")
    ...
    def spawn_robot(self):
        ...
        self.get_logger().info(f"已重生到第五赛段第一段开始位置: {future.result().status_message}")
...
def teleport_robot():
    ...
    raise RuntimeError("移动到第五赛段第一段开始位置失败；" + "；".join(failures))
